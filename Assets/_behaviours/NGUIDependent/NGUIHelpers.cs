...
namespace Bonobo
{
	public static class NGUIHelpers
	{
	    ...
	    static void AlphaInternal(Transform tra, float duration, float value, Hashtable ht)
	    {
	        ...
	        if (widget!= null)
	        {
	            TweenAlpha tweenBehaviour = tra.GetComponent<TweenAlpha>() as TweenAlpha;
	            ...
	            tweenBehaviour = tra.gameObject.AddComponent<TweenAlpha>() as TweenAlpha;

	            tweenBehaviour.from = widget.alpha;
	            tweenBehaviour.to = value;
	            tweenBehaviour.duration = duration;

	            if(ht != null)
	            {
	                if(ht.ContainsKey("method") && ht["method"] is UITweener.Method)
	                {
	                    tweenBehaviour.method = (UITweener.Method)ht["method"];
	                }

	                if(ht.ContainsKey("style") && ht["style"] is UITweener.Style)
	                {
	                    tweenBehaviour.style = (UITweener.Style)ht["style"];
	                }
	                ...
	                {
	                    tweenBehaviour.onFinished.Add(new EventDelegate((MonoBehaviour)ht["onFinishedEventReceiver"], (string)ht["onFinishedMethodName"]));
	                }
	            }
